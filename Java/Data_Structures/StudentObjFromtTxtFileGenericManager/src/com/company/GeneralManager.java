...
This program is an adaptation of "StudentObjectsFromTxtFilePt4", however, it is now built to demonstrate how one would
calculate and store all data for each student record in an array and implement a Generic Manager to sort the array

You will see a class GenericManager that accepts and stores student objects in a ArrayList and sorts them using
a generic sort method called ManageAndSort. 
...
public class GeneralManager {
    ...
    public static void main(String[] args) throws Exception{
        ...
        GenericManager<Student> gStudents = new GenericManager<Student>();
        int i = 0;
        ...
        //Sort using the GenericManager class and its generic sort
        System.out.println("---------------------------------------------");
        System.out.println("This is the Academic Class after using the Generic Manager sort:\r\n");
        //store the student records using Generic Manager
        for(i = 0; i < academicClass.length; i++) {
            gStudents.setvalue(academicClass[i]);
        }

        gStudents.ManageAndSort();
        for (i = 0; i < academicClass.length; i++) {
            System.out.println(academicClass[i]);
        }

        for(i = 0; i < academicClass.length; i++){
            gStudents.getvalue(i);
        }
...
//build Generic class type
class GenericManager<T extends Comparable<Student>>{
    ...
    //generic sort method to sort list highest to lowest
    public void ManageAndSort(){
        ...
        while(isw == 1){
            ...
            for (int i = 0; i <= mylist.size() - 2; i++){
                switch (mylist.get(i).compareTo((Student) mylist.get(i + 1))) {
                    case 1: // the objects are in the right order
                        break;
                    case -1: // the objects are in the wrong order
                        xsave = mylist.get(i);
                        mylist.set(i, mylist.get(i + 1));
                        mylist.set(i + 1, xsave);
                        isw = 1;
                        break;
                    default: // objects are equal so no change
                }
